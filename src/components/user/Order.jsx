  ...
  return (
    <div className="px-16  h-screen">
      {order.length > 0 ? (
            ...
            <div className="ml-6 mr-6 py-4">
              ...
              {item.orderDetails.length > 0
                ? item.orderDetails.map((item,index) => (
                    <div className="flex w-full mt-4 gap-2 border border-gray-200 shadow-md" key={index}>
                      ...
                      <div className="w-[500px] p-2 text-xl">ชื่อสินค้า: {item.product.name}</div>
                      <div className="w-[500px] p-2 text-xl">
                        <div>ราคา: {numeral(item.product.price).format("0,0")}</div>
                        <div>จำนวน: {item.quantity}</div>
                      </div>
                      <div className="w-[500px] text-right mr-10 py-2 text-xl">
                        ราคารวม: {numeral(item.product.price * item.quantity).format("0,0")}
                      </div>
                    </div>
                  ))
                : null}
              <div className="flex justify-between py-4">
                <div className="">
                  ...
                  <div className="text-xl">ชื่อ: {item.address.recipientName}</div>
                  <div className="text-xl">เบอร์โทร: {item.address.phone}</div>
                  <div className="text-xl">ที่อยู่: {item.address.addressDetail}</div>
                </div>
                <div>
                  <div className="mr-4 font-bold text-xl">ราคารวมทั้งหมด : {numeral(item.totalPrice).format("0,0")}</div>
        ...
        <div className="flex justify-center mt-50 font-bold text-4xl">ไม่มีรายการสั่งซื้อสินค้า</div>
      )}
    </div>
  );
};
